### Disable scheduled task for server manager displaying at startup
Get-ScheduledTask -TaskName ServerManager | Disable-ScheduledTask -Verbose

### Load the current default profile to do customize all the new users
reg.exe load HKU\DEFAULT c:\users\default\ntuser.dat

### Show Desktop Icons
$ErrorActionPreference = "SilentlyContinue"
If ($Error) {$Error.Clear()}
$RegistryPath = "HKU:\DEFAULT\Software\Microsoft\Windows\CurrentVersion\Explorer\Advanced"
...
$RegistryPath = "HKU:\DEFAULT\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons"
...
$RegistryPath = "HKU:\DEFAULT\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons\NewStartPanel"
If (-Not(Test-Path $RegistryPath)) {
	New-Item -Path "HKCU:\Software\Microsoft\Windows\CurrentVersion\Explorer\HideDesktopIcons" -Name "NewStartPanel" -Force | Out-Null
}
If (Test-Path $RegistryPath) {
